...
const QUANTITY_CHIPS = ['3', '5', '10', '20']
...
export default function CreativeExpansionBuilder() {
  ...
  const onGenerate = async () => {
    if (!task.trim() || !idea.trim() || !format.trim()) return
  ...
  const canGenerate = task.trim() && idea.trim() && format.trim()
  ...
  return (
    <div className="py-12 max-w-7xl mx-auto px-4 md:px-8">
      ...
      <div className="text-center mb-12">
        <h1 className="text-4xl md:text-5xl font-bold text-gray-900 mb-4">Creative Expansion Prompt Builder</h1>
        <p className="text-lg text-gray-600 max-w-3xl mx-auto">Ask AI to generate multiple creative options before producing the final result. Fill the fields, enhance with AI, and generate your final optimized prompt.</p>
      </div>
...
- "Suggest 3 possible plot twists before writing the story."`}
          ...
          <div className="space-y-3">
            ...
            <div className="flex flex-wrap gap-2">
              {QUANTITY_CHIPS.map((q) => (
                <button
                  ...
                  className={`px-3 py-1 rounded-full text-sm font-medium transition-colors ${quantity === q ? 'bg-primary-600 text-white' : 'bg-gray-100 text-gray-700 hover:bg-gray-200'}`}
                >{q}</button>
              ))}
            </div>
            <div className="flex gap-2">
              ...
              >{loading['Format'] ? '⏳' : '✨'} Enhance Format with AI</button>
            </div>
          </div>
          ...
          <InputCard
            ...
            value={idea}
            onChange={setIdea}
            onEnhance={() => onEnhance('Base Idea', idea, setIdea)}
            helpText="Paste the idea or describe the scenario to expand."
            multiline
            required
            loading={loading['Base Idea']}
          />
